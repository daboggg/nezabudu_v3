...
from bot.state_groups import SetupReminders
...
setup_dialog = Dialog(
    Window(
        ...
        Column(
            SwitchTo(Const("автоматическое откладывание"), state=SetupReminders.select_auto_delay, id="auto_delay"),
            SwitchTo(Const("кнопки откладывания"), state=SetupReminders.select_buttons_delay, id="button_delay"),
        ),
        state=SetupReminders.select_setting
    ),
    Window(
        ...
        Column(
            ...
            SwitchTo(Const("назад"), state=SetupReminders.select_setting, id='back1')
        ),
        getter=get_data,
        state=SetupReminders.select_auto_delay
    ),
    Window(
        ...
        SwitchTo(Const("назад"), state=SetupReminders.select_setting, id='back2'),
        getter=get_data,
        state=SetupReminders.select_buttons_delay
    ),
